...
* Adds 25 new bulk-data loaders backed by new `sportsdataverse-data` release tags, all following the existing `load_wnba_pbp()` shape (`(seasons, ..., dbConnection = NULL, tablename = NULL)`, `progressively()` decorator, optional DB write, `wehoop_data` class on the in-memory result):
