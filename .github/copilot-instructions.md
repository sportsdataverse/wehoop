...
- `@export`
- `@family` for grouping in pkgdown (e.g., "WNBA PBP Functions", "ESPN WBB Functions")
- A runnable example, formatted per source family:
  - **ESPN wrappers** (`espn_wbb_*`, `espn_wnba_*`): use a standard `@examples` block wrapped in `\donttest{}` (live-API examples). Do NOT add `@details` code blocks.
    ```r
    #' @examples
    #' \donttest{
    ...
    #' }
    ```
  - **WNBA Stats API and NCAA wrappers** (`wnba_*`, `ncaa_wbb_*`): preserve the legacy `@details`-with-fenced-code-block convention from wehoop 2.x for backward compatibility.
    ```r
    #' @details
    #' ```r
