...
Returns the players on court at a specific WBB play in long format (one
row per athlete entry across both competitors). Foundation for lineup
analysis. ESPN coverage is sparse — many plays return zero rows; the
wrapper returns a typed empty tibble in that case.

## Usage

``` r
espn_wbb_event_play_personnel(event_id, play_id, ...)
```
...
``` r
# \donttest{
  espn_wbb_event_play_personnel(event_id = 401283399, play_id = 4012833997)
#> ✖ 2026-05-19 17:29:53.260301: Failed to retrieve ESPN womens-college-basketball play personnel for event_id=401283399, play_id=4012833997
#> ✖ Args: league = "womens-college-basketball", event_id = 401283399, play_id = 4012833997
