...
`stat_type` defaults to 0 (regular-season aggregate as ESPN tags it for
finished events).

## Usage

``` r
espn_wbb_event_player_box(event_id, team_id, athlete_id, stat_type = 0L, ...)
```
...
- stat_type:
  ...
  Integer stat-type segment. Defaults to 0 (the only type commonly
  populated for finished events).
...
# \donttest{
  espn_wbb_event_player_box(event_id = 401276115, team_id = 52,
                               athlete_id = 4433404)
#> ✖ 2026-05-22 06:23:46.705154: Failed to retrieve ESPN womens-college-basketball event player box for event_id=401276115, team_id=52, athlete_id=4433404
