...
update_wnba_db() updates or creates a database with `wehoop` play by
play data of all completed and available games since 2002.
...
- datasets:
  ...
  datasets are written (one table per dataset, named `wnba_<dataset>` –
  e.g. `wnba_rosters`, `wnba_player_stats`). Valid values: `"pbp"`,
...
data for every available game back to the 2002 season and adds the most
recent completed games as soon as they are available for `wehoop`.
...
- `force_rebuild = c(2019, 2020)`: The data table with the name
  ...
  ongoing seasons because ESPN's data provider can make changes to the
  underlying data during the week.
