...
process_file <- function(r_path, man_dir = "man", dry_run = FALSE) {
  ...
  for (b in blocks) {
    ...
    rd_tabulars <- extract_tabular_blocks(rd_text)
    if (length(rd_tabulars) < length(tbls)) next  ## out of sync -- skip, don't guess
    ...
    for (idx in seq_along(tbls)) {
      ...
      repl <- gate_replacement(prefix, rd_tabulars[[idx]])
