...
pat  <- if (length(args) >= 1) args[[1]] else "espn_(wbb|wnba)_"
...
extract_examples <- function(path) {
  ...
  # Find \examples{ ... } block (single-level)
  start <- grep("^\\\\examples\\{", txt)
  if (length(start) == 0) return(NULL)
  # Find matching closing } at column 0 (roxygen always puts it there)
  end <- which(txt == "}")
  end <- end[end > start[1]][1]
  if (is.na(end)) return(NULL)
  block <- txt[(start[1] + 1):(end - 1)]
  # Strip outer \donttest{ and matching }
