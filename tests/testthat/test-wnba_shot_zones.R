## Tests for WNBA shot-zone classification helpers and public API.
##
## These test the internal (unexported) .add_shot_zones() helper — accessible
## via wehoop:::.add_shot_zones() after devtools::load_all() — and a live-gated
## smoke test for wnba_shot_zones().
##
...
# ---------------------------------------------------------------------------
# Fixture partition test — load pbp_1022400003.rds, check coverage invariants
# ---------------------------------------------------------------------------

test_that(".add_shot_zones partitions all FG rows to exactly one zone (WNBA fixture)", {
  skip_on_cran()
  ...
  expect_true("shot_zone" %in% colnames(result),
              label = "shot_zone column present after .add_shot_zones on WNBA fixture")
  ...
  # Zone distribution sanity: at least restricted_area, mid_range, above_the_break_3 expected
  expect_true(length(unique(fg_zones)) >= 3L,
              label = "at least 3 distinct zones appear in a WNBA game")
})
...
# ---------------------------------------------------------------------------
# wnba_shot_zones() — gated live test (WNBA_STATS_TESTS=1)
# ---------------------------------------------------------------------------
