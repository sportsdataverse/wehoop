...
test_that("load_wnba_player_impact accepts a multi-season vector", {
  skip_on_cran()
  skip_on_ci()
  skip_load_test()

  season <- most_recent_wnba_stats_season()
  df <- load_wnba_player_impact(seasons = c(season - 1, season))
...
test_that("load_wbb_ratings accepts a multi-season vector", {
  skip_on_cran()
  skip_on_ci()
  skip_load_test()

  season <- most_recent_wbb_season()
  df <- load_wbb_ratings(seasons = c(season - 1, season))
