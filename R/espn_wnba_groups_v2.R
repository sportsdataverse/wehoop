# espn_wnba_groups_v2.R
# Public WNBA shims for ESPN per-season group / conference endpoints.
# (Distinct from the existing site-v2 espn_wnba_groups, which targets a
...
#' @name espn_wnba_season_groups
NULL
#' @title
#' **Get ESPN WNBA Season Groups Index**
#' @rdname espn_wnba_season_groups
#' @author Saiem Gilani
#' @description
#' Returns the list of group IDs (conferences / divisions) for one
#' (WNBA season x season-type) via core-v2
...
#' }
espn_wnba_season_groups <- function(season = most_recent_wnba_season(),
                                    season_type = c(2L, 3L), ...) {
  .espn_basketball_season_groups(league = "wnba", season = season,
...
#' (WNBA season x season-type), plus `$ref` URLs to its parent group,
...
#' }
espn_wnba_season_group <- function(group_id,
                                   season = most_recent_wnba_season(),
                                   season_type = 2L, ...) {
  .espn_basketball_season_group(league = "wnba", season = season,
                                  season_type = season_type,
                                  group_id = group_id, ...)
}
...
#' for one (WNBA season x season-type x parent-group).
#'
...
#' }
espn_wnba_season_group_children <- function(group_id,
                                            season = most_recent_wnba_season(),
                                            season_type = 2L, ...) {
  .espn_basketball_season_group_children(league = "wnba", season = season,
                                           season_type = season_type,
                                           group_id = group_id, ...)
}
...
#' division) for one (WNBA season x season-type).
#'
#' @param group_id ESPN group identifier.
#' @param season Season year. Defaults to most recent WNBA season.
#' @param season_type Season-type id (2 = regular (default)).
...
#' }
espn_wnba_season_group_teams <- function(group_id,
                                         season = most_recent_wnba_season(),
                                         season_type = 2L, ...) {
  .espn_basketball_season_group_teams(league = "wnba", season = season,
                                        season_type = season_type,
                                        group_id = group_id, ...)
}
