...
# Both WBB (womens-college-basketball) and WNBA share the same JSON shape;
...
#'
#' @param league character(1). One of `"wnba"` or
...
#'
#' @return A `wehoop_data` tibble (or `NULL` on error). Columns depend on the
...
#'   when ESPN's `injuries` array is empty (common for WBB).
#'
#' @keywords internal
...
.espn_basketball_league_injuries <- function(league, season, ...) {
  stopifnot(league %in% c("wnba", "womens-college-basketball"))
...
#' @param ... Currently unused. Reserved for future argument threading.
#'
#' @return A `wehoop_data` tibble (or `NULL` on error). Expected columns
